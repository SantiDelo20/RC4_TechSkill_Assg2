...
//From https://github.com/KevinSaey/RC4_Combinatorial_2020-2021
...
public class BoundingMesh
{
    ...
    public BoundingMesh()
    {
        GameObject[] boundingMeshes = GameObject.FindGameObjectsWithTag("BoundingMesh"); //whatever is tagged boundig mesh will be added
        _colliders = boundingMeshes.Select(g => g.GetComponent<Collider>());//using ling

        Bounds meshBounds = new Bounds(); //storing
    ...
    public static Vector3Int GetGridDimensions(int voxelOffset, float voxelSize) =>
        (MeshBounds.size / voxelSize).ToVector3IntRound() + Vector3Int.one * voxelOffset * 2; ///-------grid dimentions
    ...
    public static bool IsInsideCentre(Voxel voxel)
    {
        ...
        while (Physics.Raycast(new Ray(point, Vector3.forward), out RaycastHit hit)) //Ray cast returns a bool hit / not hit
        {
        ...
        bool isInside = sortedHits.Any(kv => kv.Value % 2 != 0); //remainder of 2, chequing if its an odd number
        return isInside;
    }
